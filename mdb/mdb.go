...
func GetEmail(db *sql.DB, email string) (*EmailEntry, error) {
	...
	defer rows.Close() // needed otherwise db connection from cursor might block
	for rows.Next() {
...
func UpdateEmail(db *sql.DB, entry EmailEntry) error {
	...
	// To make this an UPSERT operation we try to first insert
	// if entry already already exist we update
	_, err := db.Exec(`
